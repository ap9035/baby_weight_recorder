...
> 📌 **背景**：後端已有 Membership 模型（owner/editor/viewer），但尚未實作邀請成員 API

| 任務 | 狀態 | 預計時間 | 說明 |
|------|------|----------|------|
| POST /v1/babies/{id}/members | ⬜ 待開始 | 0.5 天 | 邀請成員（需 owner 權限） |
| GET /v1/babies/{id}/members | ⬜ 待開始 | 0.5 天 | 列出成員 |
| PUT /v1/babies/{id}/members/{uid} | ⬜ 待開始 | 0.5 天 | 修改成員角色 |
| DELETE /v1/babies/{id}/members/{uid} | ⬜ 待開始 | 0.5 天 | 移除成員 |
| 前端：成員管理頁面 | ⬜ 待開始 | 1 天 | 顯示成員列表、邀請/移除功能 |
| 單元測試 | ⬜ 待開始 | 0.5 天 | pytest |

### 6.2 前端優化

| 任務 | 狀態 | 預計時間 | 說明 |
|------|------|----------|------|
| Baby ID 改為下拉選單 | ✅ 完成 | 0.5 天 | 自動載入寶寶列表 |
| 圖表縮放與平移 | ✅ 完成 | 0.5 天 | 滾輪縮放、拖曳平移、雙擊重置 |
| 圖表線性 X 軸 | ✅ 完成 | 0.5 天 | 正確的月齡數值對應 |
| Tooltip 顯示日期 | ✅ 完成 | 0.5 天 | 月齡三位小數、測量日期 |
